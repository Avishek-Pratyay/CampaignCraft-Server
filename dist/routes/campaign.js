...
router.get('/public', async (req, res) => {
    try {
        ...
        if ((0, memoryDb_1.isDbConnected)()) {
            ...
            // In-memory fallback
            let filtered = [...memoryDb_1.memoryCampaigns];
            if (search) {
                const s = search.toLowerCase();
                filtered = filtered.filter(c => c.title.toLowerCase().includes(s) || c.shortDescription.toLowerCase().includes(s));
            }
            if (status) {
...
router.get('/public/:id', async (req, res) => {
    try {
        ...
        if ((0, memoryDb_1.isDbConnected)() && mongoose_1.default.Types.ObjectId.isValid(idParam)) {
            const campaign = await Campaign_1.default.findById(idParam).populate('brandProfile');
            if (!campaign) {
            ...
            // In-memory fallback
            const campaign = memoryDb_1.memoryCampaigns.find(c => c._id === idParam);
            if (!campaign) {
                return res.status(404).json({ message: 'Campaign not found' });
            }
            ...
            const related = memoryDb_1.memoryCampaigns
...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if ((0, memoryDb_1.isDbConnected)()) {
            ...
            const campaigns = memoryDb_1.memoryCampaigns
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if ((0, memoryDb_1.isDbConnected)() && (!brandProfileId || mongoose_1.default.Types.ObjectId.isValid(brandProfileId))) {
            ...
            memoryDb_1.memoryCampaigns.push(newCampaign);
            return res.status(201).json(newCampaign);
        }
    }
    catch (error) {
...
router.delete('/:id', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        if ((0, memoryDb_1.isDbConnected)() && mongoose_1.default.Types.ObjectId.isValid(idParam)) {
            const campaign = await Campaign_1.default.findOneAndDelete({ _id: idParam, owner: userId });
            if (!campaign) {
            ...
            // In-memory fallback
            const idx = memoryDb_1.memoryCampaigns.findIndex(c => c._id === idParam && c.owner === userId);
            if (idx === -1) {
                return res.status(404).json({ message: 'Campaign not found or unauthorized' });
            }
            memoryDb_1.memoryCampaigns.splice(idx, 1);
            return res.status(200).json({ message: 'Campaign deleted successfully' });
        }
    }
    catch (error) {
